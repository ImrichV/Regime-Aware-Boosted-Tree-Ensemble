...
def configure_parser(parser: argparse.ArgumentParser) -> None:
    parser.description = "Candidate framework and accepted playbook builders"
    subparsers = parser.add_subparsers(dest="command", required=True)

    build_parser = subparsers.add_parser("build", help="Build candidate shards from a config")
    ...
    verify_parser = subparsers.add_parser("verify", help="Verify every candidate shard and ID")
    verify_parser.add_argument("--reference", required=True)
    verify_parser.add_argument("--output", required=True)

    publish_parser = subparsers.add_parser("publish", help="Publish a completed candidate run")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="tsr candidates")
